...
export class DemosWebAuth {
    ...
    async login(
        ...
        required(privKey, "You need to provide a private key!")
     ...
     **/
    async logout() {
        required(this.keypair, "You need to login first!")
        this.loggedIn = false
        this.keypair = null
        this.stringified_keypair = null
    ...
    async sign(message: any) {
        required(this.keypair || this.stringified_keypair, "You need to login first!")
