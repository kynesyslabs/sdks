...
describe("vLEI verifyChain (injected source)", () => {
    ...
    it("fails closed when the scope restricts corridors but the transaction omits one", async () => {
        ...
        expect(v.scope?.ok).toBe(false)
        expect(v.reasons.some(r => r.includes("restricts corridors but the transaction declares no corridor"))).toBe(true)
    })
    ...
    it("fails closed when the scope restricts networks but the transaction omits one", async () => {
        ...
        expect(v.scope?.ok).toBe(false)
        expect(v.reasons.some(r => r.includes("relying networks but the transaction declares no network"))).toBe(true)
    })
    ...
    it("rejects an operator outside the modelled I2I/NI2I set", async () => {
        const creds = baseCreds()
        creds[LE_SAID].sad.e!.qvi.o = "XYZ"
        const v = await verifyChain(mockSource(creds), AA_SAID, GLEIF_ROOT, { timestamp: FIXED_TS })
        expect(v.ok).toBe(false)
